...
//            $schedule_id = '0';
        if($this->request->getData('control_comment')){
            print_r($this->request->getData('control_comment'));die('hete');
            $control_comment = $this->request->getData('control_comment');// => on
        }
        if($this->request->getData('control_chat')){
            $control_chat = $this->request->getData('control_chat'); //=> on
        }
        if($this->request->getData('control_guest_files')){
            $control_guest_files = $this->request->getData('control_guest_files'); //=> on
        }
        if($this->request->getData('control_guest_guests')){
            $control_guest_guests = $this->request->getData('control_guest_guests');// => on
        }
        if($this->request->getData('control_notify_chat')){
            $control_notify_chat = $this->request->getData('control_notify_chat'); //=> on
        }
        if($this->request->getData('control_notify_comment')){
            $control_notify_comment = $this->request->getData('control_notify_comment');// => on
        }
        if($this->request->getData('control_notify_pictures')){
            $control_notify_pictures = $this->request->getData('control_notify_pictures');// => on
        }
        if($this->request->getData('control_notify_files')){
            $control_notify_files = $this->request->getData('control_notify_files');// => on
        }
